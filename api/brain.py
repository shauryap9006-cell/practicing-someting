...
class BrainOrchestrator:
    ...
    def advise(
        ...
        if target_station_code:
            target_stop = next(
                (s for s in route if s["station_code"] == target_station_code.upper()), route[-1]
            )
        else:
            target_stop = route[-1]
        ...
        try:
            ...
            feat_dict = {
                "current_delay": c_delay,
                "km_remaining": max(
                    0.0, float(target_stop["distance_km"]) - float(route[c_seq - 1]["distance_km"])
                ),
            ...
            try:
                ...
                dispatcher.dispatch(
                    AlertEvent(
                        ...
                        event_type="advisory",
                        title=str(top_rec.get("action_name", f"Advisory for #{train_no}")),
                        body=str(top_rec.get("reason", "Operational action required")),
                        station_code=target_stn,
                        train_no=train_no,
                        roles=["controller", "pointsman"],
                        ack_id=adv_id,
                        metadata={
                            "action_code": top_rec.get("action_code"),
                            "target_train": top_rec.get("target_train"),
                        },
                    )
                )
