...
def get_station_summary_raw(db: Database, station_code: str) -> Optional[Dict[str, Any]]:
    ...
    with db.transaction() as cur:
        cur.execute(
            "SELECT code, name, zone, platforms FROM stations WHERE code = ?", (station_code,)
        )
        ...
        cur.execute(
            "SELECT COUNT(DISTINCT train_no) AS count FROM route_stations WHERE station_code = ?",
            (station_code,),
        )
    ...
    return {
        ...
        "avg_delay": float(avg_delay_row["avg_delay"])
        if avg_delay_row and avg_delay_row["avg_delay"] is not None
        else 0.0,
