...
## 1. Executive Summary & Data Honesty Policy
Base corpus: curated real historical train events, augmented with physics-calibrated perturbations for rare cascading failures. Live demonstration positions are generated by a physics twin calibrated on the real corpus and are labeled source: simulated on every payload.
...
- **Corridor**: New Delhi (NDLS) - Ghaziabad (GZB) - Aligarh (ALJN) - Tundla (TDL) - Etawah (ETW) - Kanpur Central (CNB) - Unnao (ON) - Lucknow Charbagh (LKO) (440.0 km, 8 stations).
