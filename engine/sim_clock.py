...
from engine.clocks import TimeProvider, IST_TIMEZONE, set_global_clock, get_clock
...
class SimulatedClock(TimeProvider):
    ...
    def __init__(
        ...
        if str(st).lower() == "auto":
            ...
            self._sim_base = datetime.datetime(today.year, today.month, today.day, peak_hour, 0, 0, tzinfo=IST_TIMEZONE)
        elif isinstance(st, str):
            if ":" in st and "T" not in st and len(st.split(":")) == 2:
                hh, mm = [int(x) for x in st.split(":")]
                self._sim_base = datetime.datetime(today.year, today.month, today.day, hh, mm, 0, tzinfo=IST_TIMEZONE)
            else:
            ...
            self._sim_base = datetime.datetime(today.year, today.month, today.day, 11, 0, 0, tzinfo=IST_TIMEZONE)
    ...
    def jump_to(self, target: str | datetime.datetime) -> datetime.datetime:
        ...
        if isinstance(target, str):
            if ":" in target and "T" not in target:
                ...
                today = self.now().date()
                new_sim = datetime.datetime(today.year, today.month, today.day, hh, mm, 0, tzinfo=IST_TIMEZONE)
            else:
...
def get_sim_clock(db: Optional[Database] = None, start_time: Optional[str] = None, accel: Optional[float] = None) -> SimulatedClock:
