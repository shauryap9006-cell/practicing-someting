...
def run() -> None:
    ...
    lines += [
        ...
        "| ---- | ------------- | ------------ |",
        "| Train rows | 88,200 (21 days) | {:,} ({} months) |".format(
            mf["train_rows"],
            round(
                (
                    datetime.date.fromisoformat(split["train_cutoff"])
                    - datetime.date.fromisoformat(split["start_date"])
                ).days
                / 30.44,
                1,
            ),
    ...
    lines += [
        f"- **Overall MAE:** {m.get('overall_mae', '?'):.2f} ± {m.get('overall_mae_ci_95', '?'):.2f} min (95% CI)"
    ]
    lines += [f"- **80% Coverage:** {m.get('overall_coverage_80', '?'):.1f}%"]
    lines += [
        f"- **CV MAE:** {cv.get('cv_mean_mae', '?')} ± {cv.get('cv_std_mae', '?')} (6-fold rolling)"
    ]
    lines += [""]
    ...
    if per_class:
        ...
        for cls, vals in per_class.items():
            ...
            lines += [
                f"| {cls}{tag} | {vals['n']:,} | {vals['mae']} | {vals['coverage_80']}% | {vals['winkler']} |"
            ]
    else:
    ...
    lines += [
        f"- **n_test_direct:** {gate_mae.get('n_test_direct', '?'):,}"
        if isinstance(gate_mae.get("n_test_direct"), int)
        else ""
    ]
    lines += [""]
