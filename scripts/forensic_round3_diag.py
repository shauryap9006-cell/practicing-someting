...
with db.transaction() as cur:
    ...
    cur.execute(
        "SELECT DISTINCT train_no FROM station_events WHERE run_date < '2026-07-01' LIMIT 10"
    )
...
with db.transaction() as cur:
    ...
    if "timetable_version" in col_names or "valid_from" in col_names:
        if "timetable_version" in col_names:
            cur.execute(
                "SELECT timetable_version, COUNT(*) as cnt, MIN(run_date) as mn, MAX(run_date) as mx FROM route_stations GROUP BY timetable_version LIMIT 10"
            )
        else:
            cur.execute(
                "SELECT valid_from, COUNT(*) as cnt FROM route_stations GROUP BY valid_from LIMIT 10"
            )
        rows = cur.fetchall()
        for r in rows:
            print(f"  {dict(r)}")
...
champ_candidates = list(Path("ml/artifacts").glob("*.pt")) + list(
    Path("ml/artifacts").glob("*.pkl")
)
