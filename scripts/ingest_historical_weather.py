...
db = get_db()
print(
    "[INFO] Loading historical weather data from data/weather/corridor_historical_weather_2021_2025.csv..."
)
...
print(
    f"[INFO] Prepared {len(daily):,} daily weather records across {daily['station_code'].nunique()} stations."
)
...
with db.transaction() as cur:
    cur.execute(
        "SELECT MIN(date) as min_d, MAX(date) as max_d, COUNT(*) as cnt, SUM(fog_flag) as fog_cnt FROM weather"
    )
    r = cur.fetchone()
    print(
        f"[SUCCESS] SQLite weather table now has: {r['min_d']} to {r['max_d']} ({r['cnt']:,} rows, {r['fog_cnt']:,} fog station-days)."
    )
