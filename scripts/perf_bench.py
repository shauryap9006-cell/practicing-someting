...
def _ensure_seeded() -> bool:
    ...
    if t_count == 0:
        print(
            "[BENCH] WARNING: train 2421 not in DB -- bench will get 404s on journey/eta endpoints"
        )
...
def _stats(latencies: list, label: str) -> dict:
    ...
    print(
        f"[BENCH] {label:25s}: n={len(a):3d}  p50={p50:7.1f}ms  p95={p95:7.1f}ms  p99={p99:7.1f}ms"
    )
...
def run_bench() -> dict:
    ...
    FALLBACK_URLS = {
        ...
        "eta_2421": [
            "/v1/trains/2421/eta?target_station=CNB",
            "/api/trains/2421/eta?target_station=CNB",
        ],
    }
    ...
    results = {}
    baselines = {
        "board_live": {"p50": 24.9, "p95": 155.0},
        "journey_2421": {"p95": 155.0},
        "eta_2421": {"p95": 165.0},
    }
    ...
    print("\n[BENCH] SUMMARY")
    print(
        f"  board_live  p50={results['board_live']['p50']}ms  p95={results['board_live']['p95']}ms"
    )
    print(
        f"  journey     p50={results['journey_2421']['p50']}ms  p95={results['journey_2421']['p95']}ms"
    )
