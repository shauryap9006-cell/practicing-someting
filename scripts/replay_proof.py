...
def run_proof():
    ...
    with db.transaction() as cur:
        ...
        cur.execute(
            "SELECT seq, station_code FROM route_stations WHERE train_no = ? ORDER BY seq",
            (train_no,),
        )
        stops = cur.fetchall()
        ...
        cur.execute(
            "DELETE FROM station_events WHERE train_no = ? AND run_date = ?", (train_no, run_date)
        )
    ...
    print(
        f"Confidence Band: p10={band['best_p10_min']}m, p50={band['likely_p50_min']}m, p90={band['worst_p90_min']}m"
    )
    print(
        f"Position Mode Seq: {pos['mode_seq']}, Basis: {pos['basis']}, Confidence: {pos['confidence']}"
    )
    ...
    assert 50.0 <= p50 <= 150.0, (
        f"FAILED: Expected cascaded p50 delay in ~60-140 min range, got {p50}m"
    )
    assert pos["confidence"] > 0.0, "FAILED: position.confidence missing or zero"
    assert len(pos["candidates"]) >= 2, (
        "FAILED: position.candidates must show multiple candidate seqs"
    )
