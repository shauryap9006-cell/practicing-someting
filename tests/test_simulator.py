...
def test_same_rake_doom_tracker(sim_db: Database):
    ...
    cnb_shatabdi = [
        s for s in statuses if s.incoming_train == "12034" and s.outgoing_train == "12033"
    ]
...
def test_cascade_ripple_propagation(sim_db: Database):
    ...
    assert len(affected_trains) >= 3, (
        f"Expected cascade across >=3 trains, got {len(affected_trains)}"
    )
